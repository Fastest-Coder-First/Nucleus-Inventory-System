...
$(document).ready(function () {
    ...
    $(document).on('click', '#add', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data);
                data.forEach(store => {
                    $('#store_id').append(`<option class="getStoreProducts" value=${store.id}>${store.storeName}</option>`);
                });
            },
            error: function (error) {
                console.log(error);
    ...
    // on click ".getStoreProducts" hit api "/store/products/{store.id}" and refresh table
